...
public class ZoomToCesium extends DockLayoutPanel {
  ...
  public void flyTo(Cartographic point) {
    ...
    Animation flight = CameraFlightPath.createAnimationCartographic(scene,
        CameraFlightCartographicOptions.create().setDestination(destination)
            .setDirection(camera.getDirection()).setUp(camera.getUp())
    ...
    // IT DIES BEFORE HERE BECAUSE I TRIED COMMENTING OUT BELOW AND IT STILL
    // LOCKED OUT MY ABILITY TO CHANGE MAP LOC
    scene.getAnimations().add(flight);
  }

  public void addIconAndLabel(double lonDeg, double latDeg, double heightM,
      final String label, final int imageIndex) {

    final Scene scene = cesiumWidget.getScene();

    final Cartographic point = Cartographic
    ...
    getBillboards(new BillboardsReadyHandler() {
      ...
      public void billboardsReady(BillboardCollection billboardCollection,
        ...
        billboardCollection.add(BillboardOptions.create()
            ...
            .setImageIndex(imageIndex).setScale(scaleFactor));
        ...
        BoundingRectangle subRegionRect = textureAtlas.getTextureCoordinates()
            .get(imageIndex);
        ...
        // pixelOffsetScaleByDistance : new Cesium.NearFarScalar(1.5e2, 3.0,
        // 1.5e7, 0.5)
            );
  ...
  public Cartesian3 cartographicToCartesian(Cartographic point) {
    return cesiumWidget.getScene().getGlobe().getEllipsoid()
        .cartographicToCartesian(point);
  }
  ...
  private boolean isSameDestination(Camera left, Camera right) {
    if (left.getPosition().equals(right.getPosition())) {
      ...
      } else if (right.getDirection() != null
          && left.getDirection().equals(right.getDirection())) {
        return true;
      }
    }
    return false;
  ...
  private void getBillboards(final BillboardsReadyHandler billboardsReadyHandler) {
    ...
    if (cesiumWidget.getBillboardCollection() != null) {
      billboardsReadyHandler
          .billboardsReady(cesiumWidget.getBillboardCollection(),
              cesiumWidget.getTextureAtlas());
    } else {
      ...
      image.addLoadHandler(new LoadHandler() {
        ...
        public void onLoad(LoadEvent event) {
          ...
          if (cesiumWidget.getBillboardCollection() == null) {
            // I DON'T KNOW IF I REALLY WANT TO SET THE BILLBOARD COLLECTION AND
            // TEXTURE ATLAS ON
            // CesiumWidget. I THINK I ORIGINALLY DID IT FOR CONVENIENCE, BUT
            // MAYBE I SHOULD USE
            // an external class INSTEAD NOW?
            cesiumWidget.setBillboardCollection(BillboardCollection.create());
            cesiumWidget.setTextureAtlas(scene
                .createTextureAtlas(TextureAtlasOptions.create()
                    .setBorderWidthInPixels(0)));
          }

          BillboardCollection billboardCollection = cesiumWidget
          ...
          int[][] sprites = {
              ...
              { 31, 29, 6, 6 }, { 37, 27, 10, 10 },
              { 47, 25, 14, 14 },
              { 61, 23, 18, 18 },
              { 79, 21, 22, 22 },
              { 101, 19, 26, 26 },
              // Up-Arrows, small to large, imageIndex 19 to 24
              { 91, 84, 6, 6 }, { 81, 82, 10, 10 }, { 67, 80, 14, 14 },
              { 49, 78, 18, 18 },
              { 27, 76, 22, 22 },
              { 1, 74, 26, 26 },
              // Down-Arrows, small to large, imageIndex 25 to 30
              { 31, 66, 6, 6 }, { 37, 64, 10, 10 }, { 47, 62, 14, 14 },
              { 61, 60, 18, 18 }, { 79, 58, 22, 22 },
              { 101, 56, 26, 26 },
              // X's, small to large, imageIndex 31 to 36
              { 91, 111, 6, 6 }, { 81, 109, 10, 10 }, { 67, 107, 14, 14 },
              { 49, 105, 18, 18 }, { 27, 103, 22, 22 }, { 1, 101, 26, 26 },
              // Plus's, small to large, imageIndex 37 to 42
              { 92, 120, 5, 5 }, { 109, 109, 10, 10 }, { 107, 107, 14, 14 },
              { 105, 105, 18, 18 }, { 103, 103, 22, 22 }, { 101, 101, 26, 26 } };

          textureAtlas.addSubRegions(image, sprites);

          billboardCollection.setTextureAtlas(textureAtlas);

          billboardsReadyHandler.billboardsReady(
              cesiumWidget.getBillboardCollection(),
              cesiumWidget.getTextureAtlas());
        }
      });
